...
import { API_BASE_URL, signupService } from "../services/api";
...
export default function SignupScreen() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      // 인터셉터 정규화: { message, data } 형태
      const { data: joinUserData, message } = await signupService.signUp(
        requestData
      );
      console.log("✅ 서버 응답 성공:", { message, joinUserData });
      ...
      if (joinUserData) {
        ...
        Alert.alert(
          "알림",
          "회원가입은 완료되었지만 사용자 정보를 받아오지 못했습니다."
        );
      ...
      if (error.message.includes("Network request failed")) {
        ...
        if (status === 400) errorMessage = "입력 정보가 올바르지 않습니다.";
        else if (status === 409)
