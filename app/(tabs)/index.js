...
export default function SplashScreen() {
  ...
  const navigateIfNeeded = useCallback(
    (href) => {
      if (didNavigateRef.current) return; // 중복 방지
      didNavigateRef.current = true;
      cancelTimers(); // ← 이동 전에 타이머 모두 해제
      router.replace(href);
    },
    [cancelTimers, router]
  );
  ...
  useEffect(() => {
    typingIntervalRef.current = setInterval(() => {
      ...
      if (textIndex.current >= fullText.length && typingIntervalRef.current) {
        clearInterval(typingIntervalRef.current);
        typingIntervalRef.current = null;
      }
    }, 60);
    ...
    navTimeoutRef.current = setTimeout(() => {
      navigateIfNeeded("/login"); // ← 이동 직전에 cancelTimers가 자동 호출됨
    }, 6000);

    return cancelTimers; // 언마운트/화면전환 시 타이머 정리
  }, [navigateIfNeeded, cancelTimers, fullText]);

  // 타이핑된 텍스트를 캡션/본문으로 분리
  const captionPart = typedText.slice(
    0,
    Math.min(typedText.length, CAPTION.length)
  );
  const bodyPart =
    typedText.length > CAPTION.length
      ? typedText.slice(CAPTION.length).replace(/^\n+/, "")
      : "";
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.bottomRow}>
        <TouchableOpacity
          ...
          onPress={() => navigateIfNeeded("/signup")}>
          <Text style={styles.signup}>sign up</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={styles.loginButton}
          onPress={() => navigateIfNeeded("/login")}>
